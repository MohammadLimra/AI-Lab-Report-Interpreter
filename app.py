...
def send_otp_email(recipient_email, otp_code):
    ...
    port = 465  # 🟢 Port 465 bypasses Railway's port 587 outbound blocks
    sender_email = os.getenv("EMAIL_USER") or os.getenv("SMTP_USERNAME") or os.getenv("SMTP_SENDER")
    password = os.getenv("EMAIL_PASS") or os.getenv("SMTP_PASSWORD")

    if not sender_email or not password:
        return False, False, "EMAIL_USER or EMAIL_PASS environment variables are missing!"
    ...
    try:
        # 🟢 Use SMTP_SSL and add a 10-second timeout so it never hangs Gunicorn again
        with smtplib.SMTP_SSL(smtp_server, port, context=context, timeout=10) as server:
            server.login(sender_email, password)
            server.sendmail(sender_email, recipient_email, msg.as_string())
        
        # 🟢 Match your app.py expected return (3 values: success, dev_mode, error_message)
        return True, False, None
    except Exception as e:
        print(f"SMTP error caught: {e}")
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=False)    get_active_otp,
    extend_otp_expiry,
    load_history_for_user,
    save_history_for_user,
    delete_history_item_for_user,
