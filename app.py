...
def send_otp_email(recipient_email, otp_code):
    sender_email = os.getenv("OTP_FROM_EMAIL", "onboarding@resend.dev")
    
    try:
        # Send the email over HTTPS instead of standard SMTP ports
        params = {
            "from": sender_email,
            "to": recipient_email,
        ...
        print(f"Email sent successfully via Resend API! ID: {email['id']}")
        
        # Match your app.py expected tuple layout (success, dev_mode, error_message)
        return True, False, None

    except Exception as e:
        print(f"Resend API error caught: {e}")
